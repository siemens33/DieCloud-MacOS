...
## Что нового в 3.5.1 build 28

- Исправлена нижняя панель проигрывателя: цельный фон без серых боковых блоков.
- Исправлено скругление обложек и дефекты на углах.
- Дизайн build 25 сохранён без остальных изменений.
...
## Что нового в 3.5.0 build 25

DieCloude использует стабильную визуальную систему: фирменную монохромную тему, точное скругление обложек, мягкий hover и безопасную компактную плотность. Экспериментальные эффекты, которые зависели от нестабильной структуры SoundCloud, удалены. Штатный плеер не перестраивается — меняются только цвета и оформление.
...
## Что нового в версии 3.5.0 — исправленная сборка 23

- Исправлено скругление обложек: радиус применяется только к изображениям и их контейнерам, а не к строкам и карточкам целиком.
- Визуальный движок вынесен в отдельный `theme-engine.js`, который проще тестировать и обновлять.
- Переключатели F1 теперь управляют независимыми классами и повторно применяются после динамической навигации SoundCloud.
- Добавлены повторное обнаружение плеера, обложек и состояния воспроизведения.
- Улучшена надёжность Ambient Background, визуализатора, Focus Mode, Glass UI, анимаций и компактного режима.
- Версия приложения сохранена: **3.5.0**, номер сборки повышен до **23**.
